...
with SessionLocal() as db:
    if db.query(Department).count() == 0:
        for n in DEFAULT_DEPTS: db.add(Department(name=n))
    if db.query(Location).count() == 0:
        for n in DEFAULT_LOCS: db.add(Location(name=n))
    db.commit()
...
def create_employee(payload: EmployeeCreate, db: Session = Depends(get_db), _: None = Depends(require_admin)):
    emp = Employee(name=payload.name, qr_code_value=payload.qr_code_value,
...
def punch(payload: PunchIn, db: Session = Depends(get_db)):
    ...
    p = Punch(employee_id=emp.id, ts=ts, action=payload.action, m_number=payload.m_number,
              location_id=payload.location_id or emp.location_id, department_id=payload.department_id or emp.department_id,
